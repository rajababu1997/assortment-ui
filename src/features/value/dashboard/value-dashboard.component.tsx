 ...
 *   │── Info strip (Released · Planned · Awaiting · Approved)         │
...
export default function ValueDashboardPage() {
  ...
  const totals = useMemo(() => {
    ...
    return { released, planned, approved, todo };
  }, [rows, plans]);
...
function PeriodGroup({
  ...
  // Default expanded — the buyer almost always wants to see this month's
  // rows on first paint. Chevron lets them collapse to free up scroll real
  // estate when scanning across many months.
  ...
  return (
    ...
    >
      {/* Group header — click anywhere to toggle */}
      <div
        role="button"
        tabIndex={0}
        onClick={() => setOpen((v) => !v)}
        ...
        className="flex cursor-pointer items-center justify-between gap-2 border-b px-3.5 py-2.5 transition-colors hover:bg-[var(--color-surface-alt,#f1f5f9)]"
        style={{
          borderColor: 'var(--color-divider)',
          background: 'var(--color-surface-alt, #f8fafc)',
        }}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          {/* Highlighted plan-id chip — matches OTB/Value plan card styling */}
          <span
            className="inline-flex max-w-full items-center gap-1 truncate rounded-md border px-1.5 py-0.5 font-mono text-[10.5px] font-semibold tabular-nums"
            style={{
      ...
      <div className="flex items-center justify-between gap-2 border-t pt-2" style={{ borderColor: 'var(--color-divider)' }}>
        <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5">
          <Stat label="Budget" value={fmtMoney(row.budget, currency)} />
          {margin !== null && (
            <Stat
              label="Margin"
